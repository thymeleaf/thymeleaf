...
public class InlineJSTest extends AbstractDocumentProcessingTest {
    ...
    public void testInlineJS() throws Exception {
        ...
        {
            Document doc = processTemplate("InlineJSFacebookAuthUrlTest");

            assertNodeExists(doc, "/THYMELEAF_ROOT/html/body/div/script");
            Node node = getNode(doc, "/THYMELEAF_ROOT/html/body/div/script[1]");
            assertEquals("/**/'https://www.facebook.com/dialog/oauth?client_id=465955673787332&response_type=code&redirect_uri=http%3A%2F%2F127.0.0.1%3A8080%2Fsignin%2F%2Ffacebook'/**/", node.getTextContent().trim());
        }
    }
