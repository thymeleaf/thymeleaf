...
public abstract class AbstractLocalVariableDefinitionAttrProcessor 
    ...
    public AbstractLocalVariableDefinitionAttrProcessor() {
        super();
    }
    ...
    public final AttrProcessResult process(
        ...
        final Map<String,Object> newLocalVariables = 
            getNewLocalVariables(arguments, templateResolution, document, element, attribute, normalizedAttributeName, attributeValue);
        if (newLocalVariables == null) {
            throw new AttrProcessorException("Null variables map for \"" +
                    attribute.getName() + "\" attribute not allowed");
        }

        return AttrProcessResult.forRemoveAttribute(newLocalVariables);
