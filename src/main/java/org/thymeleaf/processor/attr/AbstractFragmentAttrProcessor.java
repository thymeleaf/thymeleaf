...
public abstract class AbstractFragmentAttrProcessor 
    ...
    public AbstractFragmentAttrProcessor() {
        super();
    }
    ...
    public final AttrProcessResult process(
        ...
        final boolean substituteInclusionNode =
            getSubstituteInclusionNode(arguments, templateResolution, document, element, attribute, normalizedAttributeName, attributeValue);
        
        final List<Node> newNodes = 
            getNewNodes(arguments, templateResolution, document, element, attribute, 
                    normalizedAttributeName, attributeValue, substituteInclusionNode);
    ...
    private final List<Node> getNewNodes(
        ...
        final AbstractFragmentSpec fragmentSpec =
            getFragmentSpec(arguments, templateResolution, document, element, attribute, attributeName, attributeValue);
        if (fragmentSpec == null) {
            throw new AttrProcessorException("Null value for \"" +
                        attribute.getName() + "\" fragment specification not allowed");
        }
        
        final Node fragmentNode = getFragment(arguments, attribute, fragmentSpec); 
    ...
    private static Node getFragment(
            ...
            final Attr attribute, final AbstractFragmentSpec fragmentSpec) {
        ...
        if (arguments.getTemplateName().equals(fragmentTemplateName)) {
            throw new AttrProcessorException(
                    ...
                    "\"" + attribute.getName() + "\" attribute, which is forbidden.");
        }
